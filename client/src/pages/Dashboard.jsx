...
const Dashboard = () => {
  ...
  const wishCount = (wishlistItems || []).reduce(
    (total, item) => total + item.quantity,
    0
  );
      ...
      <header className="bg-white shadow-lg sticky top-0 z-50 py-4 mb-4 sm:px-20">
        <div className="flex flex-wrap justify-between items-center px-2 sm:px-6">
          <img
            src="../Logo.webp"
            alt="Logo"
            className="w-36 h-16 sm:h-20 sm:w-48 sm:pl-12"
          />
      ...
      {loading ? (
        ...
        <div className=" sm:px-20">
              ...
              {showDropdown && (
                <div className="absolute mt-2 bg-white border rounded shadow-md w-48 z-10">
                  ...
                  {categories.map((cat) => (
                    <div
                      key={cat._id}
                      className={`px-4 py-2 cursor-pointer capitalize hover:bg-gray-200 ${
          ...
          <div className="px-6 mt-4 ">
            {/* Product Grid */}
            <h2 className="text-xl font-semibold mb-4 capitalize text-gray-600">
              {selectedCategory} Products
            </h2>
            {/* Best Deals Section */}
            {filteredProducts.some((item) => item.discount) && (
              <div className="px-6 mt-6 mb-8 bg-white">
                <h2 className="text-2xl font-semibold py-4 text-orange-600">
                  Best Deals
                </h2>
                  ...
                  {filteredProducts
                    ...
                    .map((item) => {
                      ...
                      return (
                        ...
                        >
                          <Link
                            to={`/${item.id}/${item.color}/${item.price}/${item.brand}`}
                          >
                            <img
                              src={item.image}
                              alt={item.title}
                          ...
                          <div className="text-sm">
                            <div className="flex flex-wrap gap-2">
                              <p className="text-gray-500 line-through pl-2">
                                ₹{item.price}
                              </p>
                              <p className="text-green-600 font-semibold flex flex-wrap">
                                <FaLongArrowAltDown className="h-5" />
                                {item.discount}% OFF
                              </p>
                            </div>
              ...
              <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {currentProducts.map((item) => {
                  return (
                    <div
                      key={item.id}
                      className="bg-white rounded-lg shadow hover:shadow-md hover:scale-105 p-2 flex flex-col justify-between"
                      data-testid="card"
                    >
                      <Link
                        to={`/${item.id}/${item.color}/${item.price}/${item.brand}`}
